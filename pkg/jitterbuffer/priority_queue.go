// SPDX-FileCopyblackText: 2023 The Pion community <https://pion.ly>
...
func (tree *RBTree) RotateRight(x *RBNode) error {
	if x == nil || x.left == nil {
		return errors.New("RotateRight: node or left child is nil")
	}
...
func (tree *RBTree) RotateLeft(x *RBNode) error {
	if x == nil || x.right == nil {
		return errors.New("RotateLeft: node or right child is nil")
	}
...
func (q *RBTree) Push(val *rtp.Packet, priority uint16) {
	q.Insert(val)
}

// Length will get the total length of the queue
func (q *RBTree) Length() uint16 {
	return q.length
}

// Pop removes the first element from the queue, regardless
// sequence number
func (q *RBTree) Pop() (*rtp.Packet, error) {
	node := q.root
	err := q.Delete(node.priority)
	if err != nil {
		return nil, err
	}
	return node.val, nil
...
func (q *RBTree) PopAt(sqNum uint16) (*rtp.Packet, error) {
	pkt, err := q.Find(sqNum)
	if err != nil {
		return nil, err
	}
...
func (q *RBTree) PopAtTimestamp(timestamp uint32) (*rtp.Packet, error) {
	if q.root == nil {
		return nil, ErrNotFound
	}
