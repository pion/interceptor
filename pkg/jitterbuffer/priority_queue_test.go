...
func TestRotations(t *testing.T) {
	...
	t.Run("RotateLeft error cases", func(t *testing.T) {
		tree := NewTree()
		root := &RBNode{priority: 5, color: black}
		tree.root = root

		// Should error when trying to rotate with no right child
		err := tree.RotateLeft(root)
		assert.Error(t, err)
	})

	t.Run("RotateRight error cases", func(t *testing.T) {
		tree := NewTree()
		root := &RBNode{priority: 5, color: black}
		tree.root = root

		// Should error when trying to rotate with no left child
		err := tree.RotateRight(root)
		assert.Error(t, err)
	})
}
