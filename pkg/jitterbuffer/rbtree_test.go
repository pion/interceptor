...
func TestPriorityQueueAsTree(t *testing.T) {
	assert := assert.New(t)
	tree := NewTree()
...
func TestRBTree(t *testing.T) {
	...
	// Verify all the elements inserted are in the tree
	for i := 0; i < 100; i++ {
		packet, err := tree.Peek(safeUint16(i))
		assert.NoError(err)
		assert.Equal(safeUint16(i), packet.SequenceNumber)
	}
}
...
func TestTreeEdgeCases(t *testing.T) {
	...
	pkt, err := pqTree.Pop()
	assert.Nil(pkt)
	assert.Error(err, "Pop from empty queue should return error")

	// PopAt from empty queue
	pkt, err = pqTree.PopAt(1234)
	assert.Nil(pkt)
	assert.Error(err, "PopAt from empty queue should return error")

	// PopAtTimestamp from empty queue
	pkt, err = pqTree.PopAtTimestamp(5678)
	assert.Nil(pkt)
	assert.Error(err, "PopAtTimestamp from empty queue should return error")

	// Peek from empty queue
	pkt, err = pqTree.Peek(1234)
	assert.Nil(pkt)
	assert.Error(err, "Peek from empty queue should return error")

	// Length should be 0
	assert.Equal(uint16(0), pqTree.Length())

	// Insert a packet and test Length
	p := &rtp.Packet{Header: rtp.Header{SequenceNumber: 42, Timestamp: 1000}}
	pqTree.Push(p)
	assert.Equal(uint16(1), pqTree.Length())

	// Peek the inserted packet
	pkt, err = pqTree.Peek(42)
	assert.NoError(err)
	assert.Equal(uint16(42), pkt.SequenceNumber)

	// PopAt with correct sequence
	pkt, err = pqTree.PopAt(42)
	assert.NoError(err)
	assert.Equal(uint16(42), pkt.SequenceNumber)
	assert.Equal(uint16(0), pqTree.Length())

	// Insert multiple packets with different timestamps
	for i := 0; i < 5; i++ {
		pqTree.Push(&rtp.Packet{Header: rtp.Header{SequenceNumber: uint16(100 + i), Timestamp: 2000 + uint32(i)}})
	}
	assert.Equal(uint16(5), pqTree.Length())

	// PopAtTimestamp for a present timestamp
	pkt, err = pqTree.PopAtTimestamp(2002)
	assert.NoError(err)
	assert.Equal(uint32(2002), pkt.Timestamp)
	assert.Equal(uint16(4), pqTree.Length())

	// PopAtTimestamp for a non-existent timestamp
	pkt, err = pqTree.PopAtTimestamp(9999)
	assert.Nil(pkt)
	assert.Error(err)
	...
	// Insert and test Pop order
	pqTree.Push(&rtp.Packet{Header: rtp.Header{SequenceNumber: 1}})
	pqTree.Push(&rtp.Packet{Header: rtp.Header{SequenceNumber: 2}})
	pkt, err = pqTree.Pop()
	assert.NoError(err)
	assert.NotNil(pkt)
	pkt, err = pqTree.Pop()
